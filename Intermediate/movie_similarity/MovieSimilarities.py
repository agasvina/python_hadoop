...
# Modified from Udemy lecture: taming big data
...
class MovieSimilarities(MRJob):
    ...
    def load_movie_names(self):
        ...
        with open("u.item") as f:
            for line in f:
                ...
                self.movieGenre[int(fields[0])] = fields[6:24]
    ...
    def mapper_sort_similarities(self, moviePair, scores):
        ...
        genre_counter = 0;
        for i in range(0, len(self.movieGenre[int(movie1)])-1):
          genres_one = self.movieGenre[int(movie1)]
          genres_two = self.movieGenre[int(movie2)]
          if(genres_two[i] == genres_one[i] and int(genres_one[i]) == 1):
            genre_counter += 1

        # sort by similar genres and then the score
        yield (self.movieNames[int(movie1)], genre_counter, score), \
    ...
    def reducer_output_similarities(self, movieScore, similarN):
        ...
        movie1, genre, score = movieScore
        for movie2, n in similarN:
            yield movie1, (movie2, genre, score, n)
